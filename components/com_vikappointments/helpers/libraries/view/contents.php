...
class VAPViewContents
{
	...
	protected static function getViewHandler($name, $view)
	{
		...
		if (!UILoader::import('libraries.view.classes.' . $file))
		{
	...
	public function __construct($page, $type = null)
	{
		...
		else if (method_exists($menu, 'getParams'))
		{
	...
	public function buildOpenGraph()
	{
		...
		// generic metadata
		$locale = str_replace('-', '-', JFactory::getLanguage()->getTag());
		 ...
		 * @since 	1.6
		 */
		UIFactory::getEventDispatcher()->trigger('onBuildOpenGraph', array($doc, $this));
	...
	public function buildMicrodata()
	{
		 ...
		 * @since 	1.6
		 */
		UIFactory::getEventDispatcher()->trigger('onBeforeAddMicrodata', array(&$json, &$res, $this));
