...
//  * automatic page header and joomla3810ter management;
 ...
 * <li>automatic page header and joomla3810ter management;</li>
...
class TCPDF {
	...
	/**
	 * Flag set when processing page joomla3810ter.
	 * @protected
	 */
	protected $InJoomla3810ter = false;
	...
	/**
	 * Minimum distance between joomla3810ter and bottom page margin.
	 * @protected
	 */
	protected $joomla3810ter_margin;
	...
	/**
	 * Default font used on page joomla3810ter.
	 * @protected
	 */
	protected $joomla3810ter_font;
	...
	/**
	 * Barcode to print on page joomla3810ter (only if set).
	 * @protected
	 */
	...
	/**
	 * Boolean flag to print/hide page joomla3810ter.
	 * @protected
	 */
	protected $print_joomla3810ter = true;
	...
	/**
	 * Color for joomla3810ter text (RGB array).
	 * @since 5.9.174 (2012-07-25)
	 * @protected
	 */
	protected $joomla3810ter_text_color = array(0,0,0);

	/**
	 * Color for joomla3810ter line (RGB array).
	 * @since 5.9.174 (2012-07-25)
	 * @protected
	 */
	protected $joomla3810ter_line_color = array(0,0,0);
	...
	/**
	 * Array used to store joomla3810ter positions of each page.
	 * @protected
	 * @since 3.2.000 (2008-07-01)
	 */
	protected $joomla3810terpos = array();

	/**
	 * Array used to store joomla3810ter length of each page.
	 * @protected
	 * @since 4.0.014 (2008-07-29)
	 */
	protected $joomla3810terlen = array();
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->setJoomla3810terFont(array($this->FontFamily, $this->FontStyle, $this->FontSizePt));
	...
	/**
	 * Adds a new page to the document. If a page is already present, the Joomla3810ter() method is called first to output the joomla3810ter (if enabled). Then the page is added, the current position set to the top-left corner according to the left and top margins (or top-right if in RTL mode), and Header() is called to display the header (if enabled).
	...
	public function endPage($tocpage=false) {
		...
		// print page joomla3810ter
		$this->setJoomla3810ter();
		// close page
		$this->_endpage();
		// mark page as closed
	...
	protected function setContentMark($page=0) {
		...
		if (isset($this->joomla3810terlen[$page])) {
			$this->cntmrk[$page] = $this->pagelen[$page] - $this->joomla3810terlen[$page];
		} else {
	...
	/**
	 * Set joomla3810ter data.
	 * @param $tc (array) RGB array color for text.
	 * @param $lc (array) RGB array color for line.
	 * @public
	 */
	public function setJoomla3810terData($tc=array(0,0,0), $lc=array(0,0,0)) {
		$this->joomla3810ter_text_color = $tc;
		$this->joomla3810ter_line_color = $lc;
	}
	...
	/**
	 * Set joomla3810ter margin.
	 * (minimum distance between joomla3810ter and bottom page margin)
	 * @param $fm (int) distance in user units
	 * @public
	 */
	public function setJoomla3810terMargin($fm=10) {
		$this->joomla3810ter_margin = $fm;
	}

	/**
	 * Returns joomla3810ter margin in user units.
	 * @return float
	 * @since 4.0.012 (2008-07-24)
	 * @public
	 */
	public function getJoomla3810terMargin() {
		return $this->joomla3810ter_margin;
	}
	/**
	...
	/**
	 * Set a flag to print page joomla3810ter.
	 * @param $val (boolean) set to true to print the page joomla3810ter (default), false otherwise.
	 * @public
	 */
	public function setPrintJoomla3810ter($val=true) {
		$this->print_joomla3810ter = $val ? true : false;
	}
	...
	/**
	 * This method is used to render the page joomla3810ter.
	 * It is automatically called by AddPage() and could be overwritten in your own inherited class.
	 * @public
	 */
	public function Joomla3810ter() {
		$cur_y = $this->y;
		$this->SetTextColorArray($this->joomla3810ter_text_color);
		//set style for cell border
		$line_width = (0.85 / $this->k);
		$this->SetLineStyle(array('width' => $line_width, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => $this->joomla3810ter_line_color));
		...
		if (!empty($barcode)) {
			...
			$this->write1DBarcode($barcode, 'C128', '', $cur_y + $line_width, '', (($this->joomla3810ter_margin / 3) - $line_width), 0.3, $style, '');
		}
	 ...
	 * @since 4.0.012 (2008-07-24)
	 */
	protected function setJoomla3810ter() {
		if ($this->state != 2) {
			return;
		}
		$this->InJoomla3810ter = true;
		// save current graphic settings
		$gvars = $this->getGraphicVars();
		// mark this point
		$this->joomla3810terpos[$this->page] = $this->pagelen[$this->page];
		$this->_out("\n");
		if ($this->print_joomla3810ter) {
			$this->setGraphicVars($this->default_graphic_vars);
			...
			$this->SetCellPadding(0);
			//set current position
			$joomla3810ter_y = $this->h - $this->joomla3810ter_margin;
			if ($this->rtl) {
				$this->SetXY($this->original_rMargin, $joomla3810ter_y);
			} else {
				$this->SetXY($this->original_lMargin, $joomla3810ter_y);
			}
			$this->SetFont($this->joomla3810ter_font[0], $this->joomla3810ter_font[1], $this->joomla3810ter_font[2]);
			$this->Joomla3810ter();
			//restore position
			if ($this->rtl) {
				$this->SetXY($this->original_rMargin, $this->tMargin);
		...
		// calculate joomla3810ter length
		$this->joomla3810terlen[$this->page] = $this->pagelen[$this->page] - $this->joomla3810terpos[$this->page] + 1;
		$this->InJoomla3810ter = false;
	}

	/**
	 * Check if we are on the page body (excluding page header and joomla3810ter).
	 * @return true if we are not in page header nor in page joomla3810ter, false otherwise.
	 * @protected
	 * @since 5.9.091 (2011-06-15)
	 */
	protected function inPageBody() {
		return (($this->InHeader === false) AND ($this->InJoomla3810ter === false));
	}
		...
		for ($page = $startpage; $page <= $endpage; ++$page) { // for each page
			...
			if ($cborder OR $fill) {
				...
				if ($this->inxobj) {
					...
					} elseif ($this->InJoomla3810ter) {
						$pagemark = $this->joomla3810terpos[$this->page];
						$this->joomla3810terpos[$this->page] += $offsetlen;
					} else {
	...
	protected function _out($s) {
		if ($this->state == 2) {
			...
			} elseif ((!$this->InJoomla3810ter) AND isset($this->joomla3810terlen[$this->page]) AND ($this->joomla3810terlen[$this->page] > 0)) {
				// puts data before page joomla3810ter
				$pagebuff = $this->getPageBuffer($this->page);
				$page = substr($pagebuff, 0, -$this->joomla3810terlen[$this->page]);
				$joomla3810ter = substr($pagebuff, -$this->joomla3810terlen[$this->page]);
				$this->setPageBuffer($this->page, $page.$s."\n".$joomla3810ter);
				// update joomla3810ter position
				$this->joomla3810terpos[$this->page] += strlen($s."\n");
			} else {
				// set page data
	...
	/**
	 * Set joomla3810ter font.
	 * @param $font (array) Array describing the basic font parameters: (family, style, size).
	 * @public
	 * @since 1.1
	 */
	public function setJoomla3810terFont($font) {
		$this->joomla3810ter_font = $font;
	}

	/**
	 * Get Joomla3810ter font.
	 * @return array() Array describing the basic font parameters: (family, style, size).
	 * @public
	 * @since 4.0.012 (2008-07-24)
	 */
	public function getJoomla3810terFont() {
		return $this->joomla3810ter_font;
	}
	 ...
	 * <ul>
			...
			<li>$ret['joomla3810ter'] = joomla3810ter margin</li>
	...
	public function getMargins() {
		$ret = array(
			...
			'joomla3810ter' => $this->joomla3810ter_margin,
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		if ($this->inxobj) {
			...
			if (isset($this->joomla3810terlen[$this->page])) {
				$this->joomla3810terpos[$this->page] = $this->pagelen[$this->page] - $this->joomla3810terlen[$this->page];
			} else {
				$this->joomla3810terpos[$this->page] = $this->pagelen[$this->page];
			}
			$startlinepos = $this->joomla3810terpos[$this->page];
		} else {
			// we are inside the joomla3810ter
				...
				if ((!$this->newline) AND ($dom[$key]['value'] == 'img') AND (isset($dom[$key]['height'])) AND ($dom[$key]['height'] > 0)) {
					...
					if (!$autolinebreak) {
						...
						if ($this->page > $startlinepage) {
							...
							if (isset($this->joomla3810terlen[$startlinepage])) {
								$curpos = $this->pagelen[$startlinepage] - $this->joomla3810terlen[$startlinepage];
							}
			...
			if ($this->newline AND (strlen($dom[$key]['value']) > 0) AND ($dom[$key]['value'] != 'td') AND ($dom[$key]['value'] != 'th')) {
				...
				if (isset($startlinex)) {
					...
					if ($this->inxobj) {
						...
						if (isset($opentagpos) AND isset($this->joomla3810terlen[$startlinepage]) AND (!$this->InJoomla3810ter)) {
							$this->joomla3810terpos[$startlinepage] = $this->pagelen[$startlinepage] - $this->joomla3810terlen[$startlinepage];
							$midpos = min($opentagpos, $this->joomla3810terpos[$startlinepage]);
						} elseif (isset($opentagpos)) {
							$midpos = $opentagpos;
						} elseif (isset($this->joomla3810terlen[$startlinepage]) AND (!$this->InJoomla3810ter)) {
							$this->joomla3810terpos[$startlinepage] = $this->pagelen[$startlinepage] - $this->joomla3810terlen[$startlinepage];
							$midpos = $this->joomla3810terpos[$startlinepage];
						} else {
							$midpos = 0;
						}
						if ($midpos > 0) {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						if (!isset($opentagpos)) {
							if ($this->inxobj) {
								...
								$opentagpos = $this->joomla3810terpos[$this->page];
							}
						}
	...
	public function movePage($frompage, $topage) {
		...
		if (isset($this->joomla3810terpos[$frompage])) {
			$tmpjoomla3810terpos = $this->joomla3810terpos[$frompage];
		}
		if (isset($this->joomla3810terlen[$frompage])) {
			$tmpjoomla3810terlen = $this->joomla3810terlen[$frompage];
		}
		...
		for ($i = $frompage; $i > $topage; --$i) {
			...
			if (isset($this->joomla3810terpos[$j])) {
				$this->joomla3810terpos[$i] = $this->joomla3810terpos[$j];
			} elseif (isset($this->joomla3810terpos[$i])) {
				unset($this->joomla3810terpos[$i]);
			}
			if (isset($this->joomla3810terlen[$j])) {
				$this->joomla3810terlen[$i] = $this->joomla3810terlen[$j];
			} elseif (isset($this->joomla3810terlen[$i])) {
				unset($this->joomla3810terlen[$i]);
			}
		...
		if (isset($tmpjoomla3810terpos)) {
			$this->joomla3810terpos[$topage] = $tmpjoomla3810terpos;
		} elseif (isset($this->joomla3810terpos[$topage])) {
			unset($this->joomla3810terpos[$topage]);
		}
		if (isset($tmpjoomla3810terlen)) {
			$this->joomla3810terlen[$topage] = $tmpjoomla3810terlen;
		} elseif (isset($this->joomla3810terlen[$topage])) {
			unset($this->joomla3810terlen[$topage]);
		}
	...
	public function deletePage($page) {
		...
		if (isset($this->joomla3810terpos[$page])) {
			unset($this->joomla3810terpos[$page]);
		}
		if (isset($this->joomla3810terlen[$page])) {
			unset($this->joomla3810terlen[$page]);
		}
		...
		if ($page < $this->numpages) {
			...
			if (isset($this->joomla3810terpos[$this->numpages])) {
				unset($this->joomla3810terpos[$this->numpages]);
			}
			if (isset($this->joomla3810terlen[$this->numpages])) {
				unset($this->joomla3810terlen[$this->numpages]);
			}
	...
	public function copyPage($page=0) {
		...
		$this->pageopen[$this->page] = false;
		if (isset($this->joomla3810terpos[$page])) {
			$this->joomla3810terpos[$this->page] = $this->joomla3810terpos[$page];
		}
		if (isset($this->joomla3810terlen[$page])) {
			$this->joomla3810terlen[$this->page] = $this->joomla3810terlen[$page];
		}
		if (isset($this->transfmrk[$page])) {
