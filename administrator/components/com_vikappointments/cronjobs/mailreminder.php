...
/**
 * The cron job is used to send a reminder e-mail to the customers.
 *
 * @since 	1.5
 * @see 	CronJob
 */
class CronJobMailReminder extends CronJob
{
	/**
	 * @override
	...
	public function doJob()
	{
		...
		$q = $dbo->getQuery(true)
			...
			->where(array(
				...
				$dbo->qn('r.mail_notified_' . $this->id()) . ' = 0',
		...
		foreach ($rows as $r)
		{
			...
			$mail_text = str_replace('{location}', $loc_str_long, $mail_text);
			$mail_text = str_replace('{location_short}', $loc_str_short, $mail_text);
			$mail_text = str_replace('{location_name}', $loc_str_name, $mail_text);
			...
			$q = $dbo->getQuery(true)
				...
				->set($dbo->qn('mail_notified_' . $this->id()) . ' = 1')
			...
			if ($dbo->getAffectedRows())
			{
				...
				$response->appendContent("Mail Cron Notification {$r['id']}-{$r['sid']} : {$identifier}");
			}	
		}
	...
	public static function getConfiguration()
	{
		...
		$f->isHtml()
			->setDefaultValue('These are all the possible accepted tags: {checkin}, {checkin_time}, {service}, {employee}, {company}, {customer}, {location}, {location_short}, {location_name}')
			->setRequired(false);
	...
	public function install()
	{
		...
		try
		{
			$q = "ALTER TABLE `#__vikappointments_reservation` ADD COLUMN `mail_notified_{$id}` tinyint(1) DEFAULT 0";
	...
	public function uninstall()
	{
		...
		try
		{
			$q = "ALTER TABLE `#__vikappointments_reservation` DROP COLUMN `mail_notified_{$id}`";
	...
	{
		return "Appointment Reminder (E-Mail)";
	}
}
