...
/**
 * The cron job is used to send a reminder e-mail to the employees.
 *
 * @since 	1.5
 * @see 	CronJob
 */
class CronJobEmployeeMailReminder extends CronJob
{
	/**
	 * @override
	...
	public function doJob()
	{
		...
		foreach ($rows as $r)
		{
			...
			$vik->sendMail($sender_mail, $company_name, $r['employee_mail'], $sender_mail, $mail_subject, $mail_text, array(), true);
			...
			if ($dbo->getAffectedRows())
			{
				...
				$response->appendContent("Employee Mail Cron Notification {$r['id']}-{$r['sid']} ({$r['nickname']}) : {$identifier}");
			}
		}
	...
	public static function getConfiguration()
	{
		...
		$f->isHtml()
			->setDefaultValue('These are all the possible accepted tags: {checkin}, {checkin_time}, {service}, {employee}, {company}, {customer}')
			->setRequired(false);
	...
	{
		return "Employee Appointment Reminder (E-Mail)";
	}
}
