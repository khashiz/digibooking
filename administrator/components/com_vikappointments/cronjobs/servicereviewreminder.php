 ...
 * to ask them if they want to leave a review for the service.
 *
 * @since   1.5
 * @see     CronJob
 */
class CronJobServiceReviewReminder extends CronJob
{
	/**
	 * @override
	...
	public function doJob()
	{
		...
		$q = $dbo->getQuery(true)
			...
			->where(array(
				...
				$dbo->qn('r.service_review_notified_' . $this->id()) . ' = 0',
		...
		foreach ($rows as $r)
		{
			...
			$url = "index.php?option=com_vikappointments&view=servicesearch&id_ser=" . $r['id_service'];
			...
			$mail_text = str_replace('{service_url}', $url, $mail_text);
			...
			$q = $dbo->getQuery(true)
				...
				->set($dbo->qn('service_review_notified_' . $this->id()) . ' = 1')
			...
			if ($dbo->getAffectedRows())
			{
				...
				$response->appendContent("Service Review Cron Notification {$r['id']}-{$r['sid']} : {$identifier}");
			}
		}
	...
	public static function getConfiguration()
	{
		...
		$f->isHtml()
			->setDefaultValue('These are all the possible accepted tags: {checkin}, {checkin_time}, {service}, {employee}, {company}, {customer}, {service_url}')
			->setRequired(false);
		...
		foreach (VikAppointments::getKnownLanguages() as $lang)
		{
			...
			$f->isInputText()
				->setDefaultValue('e4j.com - Service Review Reminder')
			...
			$f->isTextarea()
				->setDefaultValue('Please, post a review for our service from the following link {service_url}.')
	...
	public function install()
	{
		...
		try
		{
			$q = "ALTER TABLE `#__vikappointments_reservation` ADD COLUMN `service_review_notified_{$id}` tinyint(1) DEFAULT 0";
	...
	public function uninstall()
	{
		...
		try
		{
			$q = "ALTER TABLE `#__vikappointments_reservation` DROP COLUMN `service_review_notified_{$id}`";
	...
	{
		return "Service Review Reminder (E-Mail)";
	}
}
