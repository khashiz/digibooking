...
/**
 * Form field used to display a list of packages groups.
 *
 * @since 1.5
 */
class JFormFieldVappackgroup extends JFormField
{
	/**
	...
	{	
		...
		$q = $dbo->getQuery(true)
			...
			->select($dbo->qn('g.title', 'text'))
			->from($dbo->qn('#__vikappointments_package_group', 'g'))
			->order($dbo->qn('g.title') . ' ASC');
