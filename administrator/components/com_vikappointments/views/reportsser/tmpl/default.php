...
<form action="index.php" method="POST" name="adminForm" id="adminForm">
	...
	<div class="vap-charts-pool">
		...
		<?php foreach ($this->charts as $chart)
		{
			...
			{
				if (empty($line_chart_data[$stat['id_employee']]))
				{
					$line_chart_data[$stat['id_employee']] = array( 
						'label'  => '',
						'months' => array(),
						'value'  => '',
					);
				...
				$line_chart_data[$stat['id_employee']]['value'] += $stat[$chart_value_secondary];

				$label = '(' . $line_chart_data[$stat['id_employee']]['value'] . ')';

				if ($chart_value_secondary == 'earning')
				{
					if ($line_chart_data[$stat['id_employee']]['value'] > 0)
					{
						// format currency if we need to show the earning as secondary value
						$label = '(' . VikAppointments::printPriceCurrencySymb($line_chart_data[$stat['id_employee']]['value']) . ')';
					}
					else
					{
				...
				$line_chart_data[$stat['id_employee']]['label'] = $stat['ename'] . ' ' . $label;
			}
			...
			while ($date_range[0] < $this->endRange)
			{
				...
				$label = mb_substr(JText::_($date_range['month']), 0, 3, 'UTF-8') . ' ' . $date_range['year'];
				...
				foreach ($line_chart_data as $id_emp => $value)
				{
					$line_chart_data[$id_emp]['months'][$date_id] = 0;
				}
			}
			...
			foreach ($chart['line2d'] as $stat)
			{
				$line_chart_data[$stat['id_employee']]['months'][$stat['month']] = $stat[$chart_value_primary];
			...
			foreach ($line_chart_data as $id_emp => $dataset)
			{
			...
			$sid = $chart['service']['id'];
			
			?>
			
			<?php echo $vik->openFieldset($chart['service']['name']); ?>
			
				<div class="vap-empchart-box">
					
					<div class="vap-linechart-container">
						<div class="vap-linechart-wrapper">
							<canvas id="vap-linechart<?php echo $sid; ?>" class="linechart-graphics"></canvas>
						</div>
					...
					<div class="vap-piechart-container">
						<div class="vap-piechart-wrapper">
							<canvas id="vap-piechart<?php echo $sid; ?>" class="piechart-graphics"></canvas>
						</div>
					</div>
			...
			<?php if (count($chart['line2d']) > 0) { ?>
				...
				<?php foreach ($line_chart_data as $id_emp => $dataset) { ?>
				...
				var ctx = document.getElementById('vap-linechart<?php echo $sid; ?>').getContext('2d');
			...
			<?php if (count($chart['pie2d'])) { ?>
				...
				<?php foreach ($chart['pie2d'] as $dataset) { ?> 
					...
					pie_data.labels.push('<?php echo addslashes($dataset['ename']); ?>');
				<?php } ?>
				...
				var pie_ctx = document.getElementById('vap-piechart<?php echo $sid; ?>').getContext('2d');
	...
	<input type="hidden" name="task" value="reportsser" />
...
<script>
	...
	function searchButtonPressed() {
		jQuery('input[name="task"]').val('reportsser');
