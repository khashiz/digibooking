	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="<?php echo $vik->getAdminTableClass(); ?>">
		<?php echo $vik->openTableHead(); ?>
			<tr>
				...
				if ($all_fields['id'])
				{
					?><th class="<?php echo $vik->getAdminThClass('left'); ?>" width="70" style="text-align: left;"><?php echo $links[0]; ?></th><?php
				}
				if ($all_fields['sid'])
				{
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++)
		{
			...
			<tr class="row<?php echo ($i % 2); ?>">
				...
				<td>
					<span class="vapresidlistsp" style="display: inline-block;height: 16px;margin-top: 2px;" title="<?php echo $oid_tooltip; ?>"><?php echo $row['id']; ?></span>
