...
$links = array(
	OrderingManager::getLinkColumnOrder('emppayments', JText::_('VAPMANAGESERVICE1'), 'id', $ordering['id'], 1, $filters, 'vapheadcolactive'.(($ordering['id'] == 2) ? 1 : 2)),
	OrderingManager::getLinkColumnOrder('emppayments', JText::_('VAPMANAGEPAYMENT1'), 'name', $ordering['name'], 1, $filters, 'vapheadcolactive'.(($ordering['name'] == 2) ? 1 : 2)),
	OrderingManager::getLinkColumnOrder('emppayments', JText::_('VAPMANAGEPAYMENT2'), 'file', $ordering['file'], 1, $filters, 'vapheadcolactive'.(($ordering['file'] == 2) ? 1 : 2)),
	OrderingManager::getLinkColumnOrder('emppayments', JText::_('VAPMANAGEPAYMENT4'), 'charge', $ordering['charge'], 1, $filters, 'vapheadcolactive'.(($ordering['charge'] == 2) ? 1 : 2)),
	OrderingManager::getLinkColumnOrder('emppayments', JText::_('VAPMANAGEPAYMENT10'), 'ordering', $ordering['ordering'], 1, $filters, 'vapheadcolactive'.(($ordering['ordering'] == 2) ? 1 : 2)),
);
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="<?php echo $vik->getAdminTableClass(); ?>">
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++) {
			...
			<tr class="row<?php echo ($i % 2); ?>">
				...
				<td>
					<?php if ($core_edit) { ?>
						<a href="index.php?option=com_vikappointments&amp;task=editpayment&amp;cid[]=<?php echo $row['id']; ?>&amp;id_employee=<?php echo $this->id_emp; ?>"><?php echo $row['name']; ?></a>
					<?php } else { ?>
						<?php echo $row['name']; ?>
					<?php } ?>
				</td>
				...
				<td style="text-align: center;">
					<?php if ($core_edit) { ?>
						<a href="index.php?option=com_vikappointments&task=changeStatusColumn&table_db=gpayments&column_db=published&val=<?php echo $row['published']; ?>&id=<?php echo $row['id']; ?>&return_task=emppayments&params[id_emp]=<?php echo $this->id_emp; ?>">
					...
					<?php if ($core_edit) { ?>
						...
						<?php if ($ordering['ordering'] > 0) { ?>
							<?php if ($row['ordering'] > $this->bounds['min']) { ?>
								<a href="index.php?option=com_vikappointments&amp;task=sortfield&amp;cid[]=<?php echo $row['id']; ?>&amp;mode=up&amp;db_table=gpayments&amp;return_task=emppayments&params[id_emp]=<?php echo $this->id_emp; ?>&where[id_employee]=<?php echo $this->id_emp; ?>">
							...
							<?php if ($row['ordering'] < $this->bounds['max']) { ?>
								<a href="index.php?option=com_vikappointments&amp;task=sortfield&amp;cid[]=<?php echo $row['id']; ?>&amp;mode=down&amp;db_table=gpayments&amp;return_task=emppayments&params[id_emp]=<?php echo $this->id_emp; ?>&where[id_employee]=<?php echo $this->id_emp; ?>">
...
<?php } ?>
	...
	<input type="hidden" name="task" value="emppayments" />
	<input type="hidden" name="boxchecked" value="0" />
	<input type="hidden" name="id_emp" value="<?php echo $this->id_emp; ?>" />
...
<script>
	...
	function saveSort() {
		jQuery('input[name=task]').val('saveEmployeePaymentSort');
		document.adminForm.submit();
	}
