			...
			for ($h = $opening['hour']; $h <= $end; $h++)
			{
				...
				<tr>
					...
					for ($i = 0; $i < 7; $i++)
					{
						...
						//$bounds[] = strtotime($_date->format('Y-m-d H:i:s'));
						$_date->modify(($h + 1) . ':00:00');

                        $bounds[] = ArasJoomlaVikApp::jDateTimeToTimestamp($_date->format('Y-m-d H:i:s'));
                        //$bounds[] = strtotime($_date->format('Y-m-d H:i:s'));

						$rects = $cal->getIntersections($bounds[0], $bounds[1]); // matin
...
<script>
	...
	jQuery(document).ready(function() {
		...
		jQuery('td.time-empty, th.head-date').on('click', function() {
			document.adminForm.date.value = jQuery(this).find('div').data('cell-date');
