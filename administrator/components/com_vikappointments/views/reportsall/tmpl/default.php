...
<form action="index.php" method="POST" name="adminForm" id="adminForm">
	...
	<div class="btn-toolbar" style="height: 32px;">
		...
		<div class="btn-group pull-right">
			<a href="index.php?option=com_vikappointments&task=reportsallser" class="btn">
				<?php echo JText::_('VAPSEEREPORTSER'); ?>
			</a>
		</div>
	</div>
	...
	<div class="vap-charts-filter-employees">
		...
		{ 
			$selected = '';
			if (count($this->selectedEmployees) == 0 || in_array($e['id'], $this->selectedEmployees))
			{
				$selected = 'checked="checked"';
			}
			?>
			<div class="vap-charts-employee-control">
				<input type="checkbox" value="<?php echo $e['id']; ?>" name="employees[]" id="vapemployee<?php echo $e['id']; ?>" <?php echo $selected; ?>/>
				<label for="vapemployee<?php echo $e['id']; ?>"><?php echo $e['firstname'] . ' ' . $e['lastname']; ?></label>
			</div>
		<?php } ?>
	</div>
	...
	foreach ($this->lineChartStat as $stat)
	{
		...
		$line_chart_data[$stat['id_employee']]['label'] = $stat['firstname'] . ' ' . $stat['lastname'] . ' ' . $label;
	}
	...
	<input type="hidden" name="task" value="reportsall" />
...
<?php if (count($this->pieChartStat)) { ?>
	...
	<?php foreach ($this->pieChartStat as $dataset) { ?> 
		...
		pie_data.labels.push('<?php echo addslashes($dataset['firstname'] . ' ' . $dataset['lastname']); ?>');
	<?php } ?>
