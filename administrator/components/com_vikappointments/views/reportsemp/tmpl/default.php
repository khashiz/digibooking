...
JHtml::_('formbehavior.chosen', '.btn-toolbar, #dialog-confirm');
...
$ts_way = $config->getUint('tswayreport');
...
<form action="index.php" method="POST" name="adminForm" id="adminForm">
	...
	<div class="btn-toolbar" style="height: 32px;">
		...
		<div class="btn-group pull-left">
			<button type="button" class="btn" onClick="searchButtonPressed();">
	...
	<div class="vap-charts-pool">
		...
		<?php foreach ($this->charts as $chart)
		{
			...
			foreach ($chart['line2d'] as $stat)
			{
				...
				$line_chart_data[$stat['id_service']]['label'] = $stat['sname'] . ' ' . $label;
			}
			...
			$eid = $chart['employee']['id'];
			
			?>
			
			<?php echo $vik->openFieldset($chart['employee']['firstname'] . ' ' . $chart['employee']['lastname']); ?>
			
				<div class="vap-empchart-box">
					
					<div class="vap-linechart-container">
						<div class="vap-linechart-wrapper">
							<canvas id="vap-linechart<?php echo $eid; ?>" class="linechart-graphics"></canvas>
						</div>
					...
					<div class="vap-piechart-container">
						<div class="vap-piechart-wrapper">
							<canvas id="vap-piechart<?php echo $eid; ?>" class="piechart-graphics"></canvas>
						</div>
					</div>
			...
			<?php if (count($chart['line2d']) > 0) { ?>
				...
				var ctx = document.getElementById('vap-linechart<?php echo $eid; ?>').getContext('2d');
			...
			<?php if (count($chart['pie2d'])) { ?>
				...
				<?php foreach ($chart['pie2d'] as $dataset) { ?> 
				...
				var pie_ctx = document.getElementById('vap-piechart<?php echo $eid; ?>').getContext('2d');
	...
	<input type="hidden" name="task" value="reportsemp" />
...
<div id="dialog-confirm" title="<?php echo JText::_('VAPDOWNLOAD');?>" style="display: none;">
	<p>
		<span>
			<?php echo JText::_('VAPDOWNLOADREPORTMESSAGE'); ?>
			<?php
			$options = array();
			$options[] = JHtml::_('select.option', 'checkin_ts', 'VAPDOWNLOADREPORTOPT1');
			$options[] = JHtml::_('select.option', 'createdon', 'VAPDOWNLOADREPORTOPT2');
			?>
			<select id="vap-tsway-select">
				<?php echo JHtml::_('select.options', $options, 'value', 'text', $ts_way == 1 ? 'checkin_ts' : 'createdon', true); ?>
			</select>
		</span>
	</p>
</div>
...
<script>
	...
	jQuery(document).ready(function() {
		...
		VikRenderer.chosen('#vap-tsway-select');
		...
		jQuery("#dialog-confirm").dialog({
			...
			buttons: {
				"<?php echo JText::_('VAPRENEWSESSIONCONFOK'); ?>": function() {
					jQuery('#adminForm').append('<input type="hidden" name="tsway" value="'+jQuery('#vap-tsway-select').val()+'" />');
					jQuery(this).dialog("close");
					Joomla.submitform('download_employee_report', document.adminForm);
				},
	...
	function searchButtonPressed() {
		jQuery('input[name="task"]').val('reportsemp');
		jQuery('#adminForm').submit();
	}

	Joomla.submitbutton = function(task) {
		if (task == 'download_employee_report') {
			openDownloadDialog();
		} else {
