...
<form action="index.php" method="POST" name="adminForm" id="adminForm">
	...
	<div id="translation">
		<?php echo $vik->openEmptyFieldset(); ?>
			...
			<!-- DESCRIPTION - Editor -->
			<?php echo $vik->openControl(JText::_('VAPMANAGEOPTION3').':'); ?>
				<textarea name="description" style="width: 65%;height: 300px;"><?php echo (!empty($row['description']) ? $row['description'] : ''); ?></textarea>
			<?php echo $vik->closeControl(); ?>

			<!-- VARIATIONS - Text -->
			<?php 
			$vars_json = !empty($row['vars_json']) ? json_decode($row['vars_json'], true) : array();
			
			foreach ($this->vars as $i => $var) { ?>
				<?php echo $vik->openControl(JText::_('VAPMANAGERESERVATION39') . ' #' . ($i + 1) . ':'); ?>
					<input type="text" name="vars_json[<?php echo $var['id']; ?>]" value="<?php echo (!empty($vars_json[$var['id']]) ? $vars_json[$var['id']] : ''); ?>" size="40" />
				<?php echo $vik->closeControl(); ?>
			<?php } ?>
	...
	<div style="display: none;" id="original">
		<?php echo $vik->openEmptyFieldset(); ?>
			...
			<!-- VARIATIONS - Text -->
			<?php
			if (!empty($this->default['variations'])) {
				foreach ($this->default['variations'] as $i => $var) { ?>
					<?php echo $vik->openControl(JText::_('VAPMANAGERESERVATION39') . ' #' . ($i + 1) . ':'); ?>
						<input type="text" value="<?php echo $var['name']; ?>" size="40" />
					<?php echo $vik->closeControl(); ?>
				<?php } ?>
			<?php } ?>
	...
	<input type="hidden" name="id_option" value="<?php echo $this->idOption; ?>" />
	<input type="hidden" name="tag" value="<?php echo $this->tag; ?>" />
	<input type="hidden" name="task" value="saveLangOption" />
