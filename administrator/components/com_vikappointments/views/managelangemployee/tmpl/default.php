...
<form action="index.php" method="POST" name="adminForm" id="adminForm">
	...
	<div id="translation">
		<?php echo $vik->openEmptyFieldset(); ?>
			...
			<?php echo $vik->openControl(JText::_('VAPMANAGEEMPLOYEE4').':'); ?>
				<input type="text" name="nickname" value="<?php echo (!empty($row['nickname']) ? $row["nickname"] : ''); ?>" size="40" />
			<?php echo $vik->closeControl(); ?>
		
			<!-- ALIAS - Text -->
			<?php echo $vik->openControl(JText::_('JFIELD_ALIAS_LABEL').':'); ?>
				<input type="text" name="alias" value="<?php echo (!empty($row['alias']) ? $row["alias"] : ''); ?>" size="40" />
			<?php echo $vik->closeControl(); ?>

			<!-- NOTES - Editor -->
			<?php echo $vik->openControl(JText::_('VAPMANAGEEMPLOYEE11').':'); ?>
				<?php echo $editor->display('langnote', (!empty($row['note']) ? $row['note'] : ''), 400, 200, 70, 20); ?>
			<?php echo $vik->closeControl(); ?>
	...
	<div style="display: none;" id="original">
		<?php echo $vik->openEmptyFieldset(); ?>
			...
			<?php echo $vik->openControl(JText::_('VAPMANAGEGROUP2').':'); ?>
				<input type="text" value="<?php echo (!empty($this->default['nickname']) ? $this->default['nickname'] : ''); ?>" size="40" readonly />
			<?php echo $vik->closeControl(); ?>

			<!-- ALIAS - Text -->
			<?php echo $vik->openControl(JText::_('JFIELD_ALIAS_LABEL').':'); ?>
				<input type="text" value="<?php echo (!empty($this->default['alias']) ? $this->default["alias"] : ''); ?>" size="40" readonly />
			<?php echo $vik->closeControl(); ?>
		
			<!-- DESCRIPTION - Editor -->
			<?php echo $vik->openControl(JText::_('VAPMANAGEGROUP3').':'); ?>
				<textarea style="width: 100%;height: 300px;min-height: 180px;resize: vertical;" readonly><?php echo (!empty($this->default['note']) ? $this->default['note'] : ''); ?></textarea>
			<?php echo $vik->closeControl(); ?>
	...
	<input type="hidden" name="task" value="saveLangEmployee" />
