...
$links = array(
	OrderingManager::getLinkColumnOrder('coupongroups', JText::_('VAPMANAGEGROUP1'), 'id', $ordering['id'], 1, $filters, 'vapheadcolactive'.(($ordering['id'] == 2) ? 1 : 2)),
	OrderingManager::getLinkColumnOrder('coupongroups', JText::_('VAPMANAGEGROUP2'), 'name', $ordering['name'], 1, $filters, 'vapheadcolactive'.(($ordering['name'] == 2) ? 1 : 2)),
	OrderingManager::getLinkColumnOrder('coupongroups', JText::_('VAPMANAGECUSTOMF6'), 'ordering', $ordering['ordering'], 1, $filters, 'vapheadcolactive'.(($ordering['ordering'] == 2) ? 1 : 2)),
);
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="<?php echo $vik->getAdminTableClass(); ?>">
		...
		<?php echo $vik->openTableHead(); ?>
			<tr>
				...
				<th class="<?php echo $vik->getAdminThClass(); ?>" width="50" style="text-align: center;"><?php echo JText::_('VAPMENUCOUPONS'); ?></th>
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++)
		{
			...
			<tr class="row<?php echo ($i % 2); ?>">
				...
				<td style="text-align: left;">
					<a href="index.php?option=com_vikappointments&amp;task=editcoupongroup&amp;cid[]=<?php echo $row['id']; ?>">
						<?php echo $row['name']; ?>
					</a>
				</td>
				...
				<td style="text-align: center;">
					<a href="index.php?option=com_vikappointments&task=coupons&id_group=<?php echo $row['id']; ?>">
						<?php echo (empty($row['coupon_id']) ? 0 : $row['count']); ?>
					</a>
				</td>
					...
					<?php if ($core_edit) { ?>
						...
						<?php if ($ordering['ordering'] > 0) { ?>
							<?php if ($row['ordering'] > $this->bounds['min']) { ?>
								<a href="index.php?option=com_vikappointments&amp;task=sortfield&amp;cid[]=<?php echo $row['id']; ?>&amp;mode=up&amp;db_table=coupon_group&amp;return_task=coupongroups">
							...
							<?php if ($row['ordering'] < $this->bounds['max']) { ?>
								<a href="index.php?option=com_vikappointments&amp;task=sortfield&amp;cid[]=<?php echo $row['id']; ?>&amp;mode=down&amp;db_table=coupon_group&amp;return_task=coupongroups">
...
<?php } ?>
	...
	<input type="hidden" name="task" value="coupongroups"/>
...
<script>
	...
	function saveSort() {
		jQuery('input[name=task]').val('saveCouponGroupSort');
